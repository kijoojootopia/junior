...
*(보조 기능: 첫 화면의 KCII 한국 전체 화장품 수출 통계와 ExchangeRate-API 환율 표시. 관세청 품목별 수출 실적 연결은 추후 작업)*
...
화면은 왼쪽의 한국 전체 수출 통계와 오른쪽 지구본으로 구성됩니다. 왼쪽 카드는 권역 선택과 관계없이 **한국 화장품의 전 세계 수출 합계**를 표시합니다. 국가·권역별 수출액과 성장률은 아직 표시하지 않습니다.

### 첫 화면 한국 전체 통계의 출처

- 확인 페이지: [대한화장품산업연구원(KCII) → 화장품 통계 → 수출입 총괄현황](https://www.kcii.re.kr/trade/total)의 **화장품산업 / 수출금액·전년대비 증감률 / 2025년** 열과 행. 전체 산업이나 수입 수치가 아닙니다.
- 화면에 기록한 값: **2025년 수출액 11,417,952천 USD**, **전년 대비 성장률 11.4%**. 큰 숫자는 원자료의 천 USD를 달러로 환산한 뒤 억 달러 단위로 반올림해 **약 114억 달러**로 표시하고, 바로 아래에 **원자료: 11,417,952천 USD**를 함께 표시합니다. 자료 기준일은 **2026-09-15**입니다.
- `data/export/trade_stats.json`의 `global.export_thousand_usd`에 원자료의 **천 USD** 금액을 출처·기준일과 함께 저장한 고정 자료입니다. 화면을 열 때 자동으로 최신 통계를 가져오는 방식은 아니며, 출처 값이 바뀌면 확인 후 갱신해야 합니다. 화면에는 통계 연도와 출처·자료 기준일을 함께 표시합니다.
- KCII 사이트 하단에는 게시물의 무단 복제·재배포 금지 안내가 있습니다. 이 화면은 출처를 밝히고 수출액·성장률 두 지표만 기록했으며, 자동 수집이나 표 전체 재배포의 허용 여부는 확인하지 않았습니다.
- 국가·권역별 수출액·성장률과 관세 표시는 이번 단계에서 보류합니다.

`GET /api/export/trade?country=US`는 국가별 연도별 수출액과 전년 대비 성장률을 반환합니다.
지도 화면은 위 국가별 API를 호출하지 않습니다. 현재 API는 국가 코드를 필터링하며 EU/EAC 회원국 합계를 계산하지 않습니다. 이전 국가 코드(DE/FR/RU/KZ) 허용도 국가별 통계를 연결할 때 정리할 예정입니다.
...
- 첫 화면의 성장률은 KCII가 공표한 **11.4%**를 그대로 사용합니다. 국가별 API의 성장률 산식은 `(최신 연도 수출액 / 바로 전 연도 수출액 - 1) × 100`이며, 전년도 자료가 없거나 0이면 `null`입니다.
...
- `data/export/trade_stats.json`: 한국 전체 수출 통계의 출처·기준일·고정 값 저장, 국가별 통계 저장용
